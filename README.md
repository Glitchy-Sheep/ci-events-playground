...
| timeout | sleep 300, killed by timeout-minutes 1 | pass | pass | pass | conclusion timed_out, ~60-70s |
...
| broken-workflow | pass | pass | pass | pass | extra broken.yml run: startup_failure with zero jobs |
...
| job_concluded(timed_out) | `make pr-timeout` |
...
| startup_failure | `make pr-broken-workflow` |
...
- The startup_failure run of `broken-workflow` contains zero jobs: a
  job-kind watcher sees nothing, you need workflow-kind watching
  (`--kind workflow`) to observe it.
