...
запускает бота, терминал 2 - команды task из этого репо, телефон/десктоп
Telegram - наблюдение и управление подписками.

Бот построен на ghbuildrobot, поэтому каждая строка матрицы сценариев
(`task scenarios`) имеет смысл и для него. Ожидаемые сообщения ниже
...
```sh
task push PR=<n>     # свежий head: новый набор джоб поверх старых
task merge PR=<n>    # если есть 📋: "влит" + подписка на PR уходит из /subscriptions
```

## Сессия 2: хвост лога при падениях - главная

```sh
task pr-fail-late                          # ошибка в конце 40k-строчного лога
task scenario PR=<n> SCENARIO=fail-early   # ошибка в начале
task scenario PR=<n> SCENARIO=fail-middle  # panic в середине
```
...
терминала: `task rerun PR=<n>`. Отдельно проверь путь с read-only
...
и это первый тест: привяжи логин, сделай `task comment`/`task review` и
убедись в тишине. Потом отвяжи логин и прогони:

```sh
task review PR=<n>              # "ревью": вердикт comment + текст
task review-comment PR=<n>      # inline-комментарий: file:line + ссылка на тред
task reply PR=<n> COMMENT=<id>  # ответ в треде - самый сложный кейс детекции
task draft PR=<n> && task ready PR=<n>   # "переведен в черновик" / "готов к ревью"
task close PR=<n>               # "закрыт без мержа"; 📋-подписка на PR остается
task reopen PR=<n>              # "переоткрыт" - подписка продолжает работать
task merge PR=<n>               # "влит" + кто влил; подписка retired
```
...
```sh
task pr-timeout        # джоба убита таймаутом: conclusion cancelled, "упала · таймаут"/🚫
task pr-cancel-me      # затем task cancel PR=<n>: 🚫 отмена на FE, ✅ BE на том же head
task commit-status PR=<n> STATE=pending   # KindStatus входит в jobs-подписку
task commit-status PR=<n> STATE=error     # терминальный сразу при обнаружении (U-12),
                                          # без лога, rerun: "на стороне CI"
task pr-broken-workflow  # ран без джоб: jobs-подписка молчит - это правильно
task pr-dual-workflow    # два "Build FE" с разными job ID: должны прийти ОБА
                         # (зеленый и красный), без слипания дедупом
task setup-env && task pr-approval   # ран висит в waiting: как бот покажет гейт?
task approve RUN=<id>                # или task reject RUN=<id> (даст failure)
```

## Сессия 6: нагрузка и статус

```sh
task storm N=5        # много PR у автора за один цикл опроса: снапшоты,
                      # backfill-стоимость подписки (U-02), очередь доставки
task pr-all-fail      # шторм падений: 4 сообщения с логами разом
```
...
```sh
task cleanup          # закрыть все sbx/* PR и удалить ветки
```
